...
xFlow turns selected X/Twitter accounts into a private, queryable feed pipeline. It fetches posts with locally stored browser auth, deduplicates them in PostgreSQL, then exposes the result through RSS, JSON, and Telegram. A daily LLM-powered digest summarizes each account's posts.
...
- **Daily LLM digest**: a scheduled, per-account daily summary generated by an LLM and delivered to Telegram.
...
- RSS feeds for all posts or one account.
